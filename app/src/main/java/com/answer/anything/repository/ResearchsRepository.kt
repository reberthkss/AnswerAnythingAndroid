...
class ResearchRepository() {
    ...
    suspend fun read(uid: String) =  withContext(Dispatchers.IO) {
        ...
        val researchs = response.documents.map {
            Research.from(it)
        }
        researchs
    }
    ...
    suspend fun findById(researchId: String) = withContext(Dispatchers.IO) {
        val response = researchCollection
            .whereEqualTo("id", researchId)
            .get()
            .await()
        Research.from(response.documents[0])
    }
